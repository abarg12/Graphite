...
let translate decls = 
  ...
  let graph_t = L.named_struct_type context "graph_t" in
    let _ = L.struct_set_body graph_t
      [|
        list_t
      |]
    in *)
...
let rec expr (builder, stable) ((styp, e) : sexpr) = match e with
  ...
  (* | SDotCall(oname, mname, sargs) ->
    (* str, str, sexpr list *)
    (* g.add([sexprs]) *)
    let sarg = match sargs with
      [x::xs] -> xs
    in
    (* let dty, nname = match sarg with
        [(_dty, SId(_nname)) -> _dty, _nname
      | _ -> raise (Failure("codegen/dotcall 1"))
    in *)
    let node_ptr = expr (builder, stable) sarg in
    let graph_ptr = find_variable stable oname in
    let vlist_ptr = L.build_struct_gep graph_ptr 0 (oname ^ ".vertex_list_ptr") builder in
          ...
          let init = match typ with
            ...
            | A.Node(typ) -> L.const_named_struct node_t
                                       [| (L.const_int i8_t 0); 
                                          ...
                                          (L.const_int i8_t 0); |] 
            | A.List -> L.const_pointer_null (L.pointer_type i8_t)
            (* | A.Graph(_) -> L.const_named_struct graph_t [|(L.const_pointer_null (node_t)) |] *)
            | _ -> raise (Failure "no global default value set")
          in 
          let new_glob = L.define_global s init the_module in
          let stable' = bind_var stable s new_glob in
